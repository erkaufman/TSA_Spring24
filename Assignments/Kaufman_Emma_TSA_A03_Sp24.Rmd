...
```{r, loading and installing packages}
...
```
The detrended series for total renewables has shifted down, the intercept is now close to zero, and the range has decreased (the difference between the maximum and minimum values). 

The detrended series for hydroelectric power has also shifted down with the intercept being closer to zero. The data seem to have a mean of around 0 now.
...
> The renewable graph changed a little bit, because we removed a signficant amount of variation from the data when removing the trend. We see really weak seasonality in the renewable ACF detrended data becuse there are very small spikes at 12, 24, 36. 
> For the hydro graphs we dont have a strong upward or downward trend, and see strong seasonaility in the ACF, for the pacf we see the highest magnitude partial autocorrelation at consistently spaced lags (13, 25, 37) which indicates seasonal component. 
...
```
Renewables intercept changed, but the graph is nearly parallel the original at any given timestep. 
The hydro graph intercept also changed, and there is less variation between peaks and troughs in the data without the sesaonal component than the data with the seasonal component. 
...
The renewable plots do not change much. makes sense because there is very little variation caused by seasonality, as confirmed by the r2 value. 
