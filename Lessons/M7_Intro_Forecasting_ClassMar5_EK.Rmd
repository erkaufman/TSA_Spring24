...
Model1 <- meanf(ts_electricity_price[,"All.sectors"], h=12) #h is forescating horizon
summary(Model1) #simple mean, all forecasts are the same and equal to the mean of the series
plot(Model1) 
```

> bad results! because we have trend and seasonality, we didn't account for this
...
summary(Model2)
plot(Model2) #average mean doesnt work if you have trend on your dataset because you wont replicate the trend

#Model 3: Seasonal naive on original data
Model3 <- snaive(ts_electricity_price[,"All.sectors"], h=12) 
summary(Model3)
plot(Model3) #replicating the last 12 of the original data, actually looks better than the mean!

#Model 4: Naive on deseas data
Model4 <- naive(deseasonal_allsectors_price, h=12) 
summary(Model4)
plot(Model4)  #looking at the last year without seasonality
```
...
plot(Model8)
#not very good at incorporating seasonality, generate forecast look at the error, and fix next forecast?? for just historical data?

autoplot(ts_electricity_price[,"All.sectors"],
         series="original") +
  autolayer(Model8$fitted, series= "ES-Model8") #errors for historical data are low, but how good can the model keep up with forecasting if were not updating parameters? 
...
autoplot(deseasonal_allsectors_price,
  ...
  autolayer(Model10_for$mean,series="SARIMA_for")
#replicated trend into the future! smaller errors (because we have removed variability by removing seasonal component), not fair to compare errors w arima and sarima because they have different variability! 
