...
(contain more terms to model more varied frequency data)

-   ETS
-   ARIMA with dynamic harmonic fourier components- can't use sarima because it can only handle one frequency. But with ARIMA we can add seasonality with external regressors. Based on sine and cosine functions
-   TBATS- exponential smoothing method under state space framework
-   Neural Network- allows for nonlinear relationship among current and previous observations of the same series to generate future forecast.
...
act_power_hourly <- act_power_all %>% 
  ...
  summarise( hourly_mean_active_power = mean(Global_active_power)) #taking the average for an entire hour (summarising based on the last column listed)
...
act_power_daily <- act_power_hourly %>% 
  ...
  summarise( daily_mean_active_power = mean(hourly_mean_active_power)) #take the mean for the day
...
#no NAs after aggregation, we would need to use TSA clean if there were still NAs after this step
```

## Transforming both series into time series objects

Let's use `msts` on our two aggregated data sets.
`msts` stands for multiple **s**easonal **t**ime **s**eries.
...
-   **ts.frequency** seasonal period that should be used as frequency of the underlying ts object. The default value is max(seasonal.periods) (only specified if main source of seasonality is not from larger frequency)\
...
ts_act_power_hourly <- msts(act_power_hourly$hourly_mean_active_power, 
                            seasonal.periods =c(24,168,8766), #hourly data seasonal periods
                            start=c(2006,12,16))

ts_act_power_daily <- msts(act_power_daily$daily_mean_active_power, #daily data frame mean
...
n_for = 365 #check forecast performance for a year 
ts_act_power_daily_train <- subset(ts_act_power_daily,
                                   end = length(ts_act_power_daily)-n_for) #removing the last year

#create a subset for testing purpose
ts_act_power_daily_test <- subset(ts_act_power_daily,
                                   start = length(ts_act_power_daily)-n_for) #only using the last year
...
ETS_fit <-  stlf(ts_act_power_daily_train,h=365) #calling the stlf for our training series, and our forecasting horizon is 365 days (same length as test)
#additive error, no trend or seasonal component on the ETS (already represented on the stl)
#fitting and forecasting at the same time
...
#sarima can't handle multiple seasonality. external regressors will show multiple seasonality
#no transformation because lambda = 0
ARIMA_Four_fit <- auto.arima(ts_act_power_daily_train, 
                             seasonal=FALSE, 
                             lambda=0, 
                             xreg=fourier(ts_act_power_daily_train, 
                                          K=c(2,12))) #exageous regressors that represent the seasonal component, functions of sine and cosine. Fourier function finds terms that fit the data, it needs information about seasonal patterns. K contains weekly and monthly harmonics (weekly is 2: one for weekday and one for weekend), (monthly is 12: one for each month), 
                             #can change these values to try and get a better fit, for example if yearly seasonality could be split into four seasons... 

#Forecast with ARIMA fit
#also need to specify h for fourier terms
ARIMA_Four_for <- forecast(ARIMA_Four_fit, #forecasting the function fit above
                           xreg=fourier(ts_act_power_daily_train,
                                        K=c(2,12), #fit
                                        h=365), #365 forecasts of the fourier terms, within the forecast function, to get forecasts of the fourier terms (must be the same as the auto.arima)
                           h=365 #specified for the forecast function 
                           ) 
...
NN_fit <- nnetar(ts_act_power_daily_train,
                 p=1, #AR component
                 P=0, #S is in fourier coefficients, don't need it here 
                 xreg=fourier(ts_act_power_daily_train, 
                              K=c(2,12))) #could colbind any other vector you are adding to the dataset, k
#NN_for <- forecast(NN_fit, h=365) 
NN_for <- forecast(NN_fit, h=365,xreg=fourier(ts_act_power_daily_train, 
                                          K=c(2,12),h=365)) #need to make sure you are also forecasting any regressors 
#h variables generates forecasts and how many desired, should match h from overall function 
...
```{r}
#checking accuracy of forecasting performance (we know that they do a good job representing historical data)
#Model 1: STL + ETS
