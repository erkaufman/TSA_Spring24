...
Model_111 <- Arima(deseasonal_residential_price,order=c(1,1,1)) #(AR order, # of differences, MA order)
print(Model_111) #get the coefficients for y(t) and y(t-1) is 0.37, the coefficients of error for a(t) and a(t-1) is 0.36
compare_aic <- data.frame(Model_111$aic)

#Model_111$fitted #estimates from model
...
plot_grid(
  ...
  autoplot(Acf(Model_111$residuals,lag.max=40, plot = FALSE)), #no serial correlation or time dependence of errors (insignificant coefficients)
...
print(Model_211)
#drift is the intercept term (subtract the mean when we difference, but if there is anything left over it will in the drift)
compare_aic <- data.frame(compare_aic,Model_211$aic) #want the aic to be the lowest value possible
...
plot_grid(
  ...
  nrow=1
) #we see some significant coefficients outside the blue line, so we can keep incorperating more terms! 
...
Model_autofit <- auto.arima(deseasonal_residential_price, max.D=0, max.P = 0, max.Q=0 ) #auto.arima tries to fit seasonality to your data, so we are forcing all of our seasonal terms to be 0 (capital terms are for seasonal component)
print(Model_autofit) #gives us same result we got from the trial and error! 
...
Model_101 <- Arima(deseasonal_residential_price,order=c(1,0,1)) #Arima function tells you it is a non-stationary series (you need to difference!)

#You will get an error!

Model_101_diff=Arima(deseasonal_residential_price_diff,order=c(1,0,1)) #we are providing an already differenced series
print(Model_101_diff)
#ma1 gives you relationship with y(t) and a(t-1), relationship between current term and residuals (-0.37). These values should match what we see in the ACF lag 1 for the MA (if we have both MA and AR then maybe it doesnt align, or if our sample isn't large enough it wont match the population. You should get closer to the population as you increase the number of samples). 
...
```{r}
#is there another way to visualize what we are doing?
autoplot(deseasonal_residential_price,series="Observed")+
  ...
  autolayer(Model_011$fitted, series = "ARIMA (0,1,1)") 
    #add fitted values from our models
