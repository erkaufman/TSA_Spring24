...
```{r}
#randomly removing 2 rows
drop_rows_price <- electricity_price_processed %>% 
  select(Month,Residential) %>% 
  filter(Month != "2001-07-01" & Month != "2005-07-01")
# now when we look at these data there are no NAs... but we still have missing data!

#Create TSA object
ts_drop_rows_price <- ts(drop_rows_price$Residential, frequency=12)
tail(ts_drop_rows_price) #we see the last value is september, but it should be novemeber! the TS didn't know that we were missing data.
...
colnames(Days) <- "Month" #same col name as og dataset 

#merge with the data with missing rows
new_price <-left_join(Days, drop_rows_price, by="Month") #merging by the same column, and all of the data in the left df will be included! we can use na.approx to estimate the missing data. 
...
residential_price_seas_diff <- diff(ts_electricity_price[,"Residential"],lag=12, differences=1) #preform differencing once at seasonal lag 12
residential_price_trend_diff <- diff(ts_electricity_price[,"Residential"],lag =1, differences=1) #diff done on orig series (lag 1)
residential_price_both_diff <- diff(residential_price_trend_diff,lag =12, differences=1) #taking the differenced at lag 1 series and differencing again
...
#is pacf lag 1 is close to 1 then you might have a unit root (stoachastic trend), need to difference the series! 
...
print(Model2)
#slightly better than our model from before (lower AIC), but similar 
...
> with original series you dont need to worry about adding the seasonal component back. When you do remove it you're assuming that the mean is constant over time
>SARMIMA allows the seasonal component to be modeled by the sarima equations and can account for variation (something that isn't constant over time)
> need to understand how to remove seasonality because not every model allows for seasonality (some optimization models can't handle temporal tendincies )
