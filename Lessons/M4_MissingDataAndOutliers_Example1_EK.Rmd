...
#look for values that are outside of a normal distribution 
#see that the data are not following a normal distribution, it is a log normal distribution 
#skewed and there are no negative values
#(but the models can't take the log transformed data)
...
#boxplots give us quantile ranges to visualize outliers
#see that HP1 has a clear outlier
```
...
```{r}
#important to decompose the series first before we detect outliers, removing seasonality and trend

#Using R decompose function
iHP=1
<<<<<<< HEAD:Lessons/M4_MissingDataAndOutliers_Example1_EK.Rmd
decompose_inflow_data <-decompose(ts_inflow_data[,iHP],"additive") #input needs to be a TS
plot(decompose_inflow_data) 
#output has all the components:
# x is original TS, seasonal, trend, and random components as well. 
#trend is not linear, so decomposition uses a moving trend decomposition (centered on your specified frequency, ex. 12)
#seasonal component uses seasonal mean model (avg of all jan, feb, march, makes a series based on those averages), decompose can only fit constant seasonal patterns! (wouldnt be good for changing seasonal patterns)
#random is og-trend-seasonal 
#additive: sum trend+seasonal+random= observed
#multiplicative: multiply all series together, use this when you think the seasonal component magnitude is a function of time (peaks increase or decrease with time)
#choose between the two based on randomness of random component, shouldn't be centered or have clear seasonality 


#Inspect random component for extreme values (outliers)
#calculate statistics to understand +/- 3 sd from mean
=======
decompose_inflow_data <- decompose(ts_inflow_data[,iHP],"additive")
plot(decompose_inflow_data)
>>>>>>> e4213812a97db48c087fe2cdde82d932cc27b25d:Lessons/M4_MissingDataAndOutliers_Example1.Rmd
...
#you end up with NAs that you didn't have before when you use the moving average, means you need to start and end your series at frequency/2 (ex 6 months after and before)

#Compute mean and standard deviation without missing values
mean_inflow <- mean(na.exclude(inflow_random))  #exclude NA or missing observation from tails to compute mean and std
sd_inflow <- sd(na.exclude(inflow_random))

cat(mean_inflow,sd_inflow)

#plotting random component: could add lines to see which observations fall outside the outiers
autoplot(inflow_random)+
  autolayer(tsclean(inflow_random))

#clean removes outliers from your series, only from the random component automatically, it removes outliers and replaces them with local mean
#if we are sure that the outliers are an error, then we can get rid of them! 
...
#Just remove them since they are on tails (tsclean can remove these automatically as well)
...
ggplot(df_inflow_random, aes(x=month, y=inflow)) +
            geom_boxplot()
#where are we seeing most of the outliers? see they occur in shifts from wet to dry seasons (this is changing with time, and not represented in our average seasonal trend)

ggplot(df_inflow_random, aes(x=date, y=inflow)) +
            geom_line() +
            geom_abline(slope=0,intercept=3*sd_inflow,color="red") +
            geom_abline(slope=0,intercept=-3*sd_inflow,color="red")
#see extreme positive values, not negative (because coming from log normal distrubtion)
#(tsclean would remove all of these values )
...
```{r}
#Just find the most extreme value, inspecting one by one
...
print(grubbs_test) #look at the p-value to find the decision (see there is enough evidence that this is an outlier)

#If you need to remove outlier use rm.outlier()
df_inflow_random$inflow <- rm.outlier(df_inflow_random$inflow,fill=TRUE) 
#using fill equal true the value will be replaced by mean or median 
...
#Loop while to remove all outliers, essentially what is within tsclean() function
#advantage here is that you can adjust p-value sensitivity 
