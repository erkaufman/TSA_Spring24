...
inflow_data <- cbind(my_date,raw_inflow_data[,3:5]) #cbind stands for column bind, no longer raw!
head(inflow_data)
...
#initial plot to look for outliers or initial observations
ggplot(inflow_data, aes(x=my_date, y=HP1))+
  geom_line(color='blue')

#adding two time series to the same plot
ggplot(inflow_data, aes(x=my_date))+
  geom_line(color='blue', aes(y=HP1))+
  geom_line(color='green', aes(y=HP2))
```
...
```{r}
#timeseries objects have more information that dataframes (starting date, the frequency of the data)
ts_inflow_data <- ts(inflow_data[,2],start=c(1931,1),frequency=12) #start= c() means were providing a vector
ts_inflow_data

ts_inflow_data_all <- ts(inflow_data[,2:4],start=c(1931,1),frequency=12) 
ts_inflow_data_all
```
...
#acf is autocorrelation function, time or self correlation of our timeseries, how is one observation correlated with another observation of the same variable 
#acf needs ts object, and acf has year units as x axis
#Acf has month units as x axis, and lag of 12 represents 12 months 

#there is influence in the coefficients based on previous lags, how do we eliminate the influence of intermediate variables in autocorrelation values? 
#then we have the direct relationship between y(t) and y(t-h)
HP1_acf <- acf(ts_inflow_data)
HP1_acf$acf #to access the values

#our plot shows us that there is a periodic pattern to the autocorrelation
#each lag represents a month
#partial autocorrelation eliminates intermediate relationships
