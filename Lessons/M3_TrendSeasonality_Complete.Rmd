...
inflow_data_matrix <- matrix(ts_inflow_data[,iHP],byrow=FALSE,nrow=12) #populate matrix by column
inflow_data_yearly <- colMeans(inflow_data_matrix) #mean for each column
...
#null hypothesis is saying no trend, data is stationary, our pvalue is less than 0.05, so we are rejecting null that data are stationary
#can conclude we have a trend in our data
#s value is positive, so we have an increasing trend 
...
#again we have a trend, and its an increasing trend (positive score) 
#s depends on number of observations (why we see differences in magnitude)
...
#reject the null, conclude that we have a trend stationary (now we need to runn Mann Kendall to confirm if there is a trend)
...
#reject null, conclude that we ahve a trend stationary (now we need to runn Mann Kendall to confirm if there is a trend)
```
